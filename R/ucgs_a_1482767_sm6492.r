...
run.ss <- function(X,Y,w,kappa,zeta,K,M,outfile.stem,fast=F,keep.beta=T,write.each=F){	
	XTY <- drop(t(X)%*%Y)
	n <- dim(X)[1]
	p <- dim(X)[2]
...
############################################################
# Generate MCMC output for Figure 1
############################################################
...
# Random seed (digits 25-27 of pi)
set.seed(383)
...
for(i in 0:(n.runs-1)){
  ...
  outfile.stem.orig <- paste("ss-acvp-orig-",runtext,sep="")
  run.ss(X,Y,w,kappa,zeta,K,M,outfile.stem.orig,fast=F,keep.beta=F,write.each=F)
  outfile.stem.fast <- paste("ss-acvp-fast-",runtext,sep="")
  run.ss(X,Y,w,kappa,zeta,K,M,outfile.stem.fast,fast=T,keep.beta=F,write.each=F)
...
K.burn <- round(K*burn)
ac.orig.single <- rep(NA,n.runs*M)
ac.fast.single <- rep(NA,n.runs*M)
chain.run.id <- rep(1:n.runs,each=M)
rundigits <- max(1,ceiling(log(n.runs,10)))  	# digits needed for run label strings
chaindigits <- max(1,ceiling(log(M,10)))		# digits needed for chain label strings
setwd(outputdir)
for(i in 0:(n.runs-1)){
  ...
  for(j in 0:(M-1)){
    ...
    filename.orig.ij <- paste("ss-acvp-orig-",runtext,"-",chaintext,"-s.txt",sep="")
    sigma2.chain.orig.ij <- read.table(filename.orig.ij)[-(1:K.burn),1]
    ac.orig.single[i*M+j+1] <- acf(sigma2.chain.orig.ij,lag.max=1,plot=F)$acf[2]
    filename.fast.ij <- paste("ss-acvp-fast-",runtext,"-",chaintext,"-s.txt",sep="")
    sigma2.chain.fast.ij <- read.table(filename.fast.ij)[-(1:K.burn),1]
    ac.fast.single[i*M+j+1] <- acf(sigma2.chain.fast.ij,lag.max=1,plot=F)$acf[2]
  }
}
setwd("..")
ac.orig <- tapply(ac.orig.single,chain.run.id,mean)
ac.fast <- tapply(ac.fast.single,chain.run.id,mean)

ndigits <- max(1,ceiling(log(max(n.),10)))		# digits needed for n label strings
min.ac <- min(ac.orig,ac.fast,0)
for(n.plot in n.){
  ntext <- substring(format(n.plot/(10^ndigits),nsmall=ndigits),3)
  plotfilename <- paste("ss-acvp-",timestamp,"-n",ntext,".pdf",sep="")
  plot.which <- n.==n.plot
  pdf(plotfilename,width=5,height=5)
  par(mar=c(4,4,1,1),mgp=c(2.75,1.00,0),pty="s")
  plot(p.[plot.which],ac.orig[plot.which],pch=1,type="b",ylim=c(min.ac,1),
       cex=1.4,cex.axis=1.4,cex.lab=1.4,xaxt="n",
       xlab=expression(italic(p)),ylab="Autocorrelation")
  axis(1,n.plot*2*(1:5),cex.axis=1.4)
  points(p.[plot.which],ac.fast[plot.which],pch=2,type="b")
  legend(1.032*max(p.[plot.which]),(min.ac+1)/2,xjust=1,yjust=0.5,pch=c(1,2),
         c("Original","Fast"),cex=1.4)
  dev.off()
}
...
############################################################
# Generate plots for Figure 2 using MCMC output generated by code above
############################################################
...
for(n.plot in n.){
	ntext <- substring(format(n.plot/(10^ndigits),nsmall=ndigits),3)
	plotfilename <- paste("acvp-",timestamp,"-n",ntext,".pdf",sep="")
	plot.which <- n.==n.plot
	pdf(plotfilename,width=5,height=5)
	par(mar=c(4,4,1,1),mgp=c(2.75,1.00,0),pty="s")
	plot(p.[plot.which],ac.orig[plot.which],pch=1,type="b",ylim=c(min.ac,1),
...
############################################################
# Generate additional MCMC output for left panel of Figure 3
# (which also uses output already generated for Figure 2)
############################################################
...
############################################################
# Generate plot for left panel of Figure 3
# using MCMC output generated by code above
############################################################
...
############################################################
# Generate MCMC output for center/right panels of Figure 3
############################################################

# n and p settings
n. <- rep(c(50,150,250,350,450),each=5)
p. <- rep(c(50,150,250,350,450),5)
...
############################################################
# Generate plots for center/right panels of Figure 3 using
# MCMC output generated by code above
############################################################
...
plotfilename.orig <- paste("dacf-",timestamp,"-orig.pdf",sep="")
pdf(plotfilename.orig,width=5,height=5)
par(mar=c(4,4,1,1),mgp=c(2.75,1.00,0),pty="s")
persp(x=unique(n.),y=unique(p.),z=matrix(ac.orig,nrow=length(unique(n.)),byrow=T),
		theta=45,phi=30,shade=0.5,zlim=c(min.ac,1),
		xlab=expression(n),ylab=expression(p),zlab="Autocorrelation",
		cex=1.4,cex.axis=1.4,cex.lab=1.4)
dev.off()
plotfilename.fast <- paste("dacf-",timestamp,"-fast.pdf",sep="")
pdf(plotfilename.fast,width=5,height=5)
par(mar=c(4,4,1,1),mgp=c(2.75,1.00,0),pty="s")
persp(x=unique(n.),y=unique(p.),z=matrix(ac.fast,nrow=length(unique(n.)),byrow=T),
		theta=45,phi=30,shade=0.5,zlim=c(min.ac,1),
		xlab=expression(n),ylab=expression(p),zlab="Autocorrelation",
		cex=1.4,cex.axis=1.4,cex.lab=1.4)
dev.off()
