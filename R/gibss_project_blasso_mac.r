...
pacman::p_load(lars, tidyverse, foreach, coda, glmnet, doMC)
...
run.bl <- function(X,Y,lambda,K,M,fast=F){
  ...
  foreach(chain = 0:(M-1), .combine = rbind) %dopar%{
...
for(i in 0:(n.runs-1)){
	...
	#--- original blasso
	ress_fast <- run.bl(X,Y,lambda,K,M,fast=F)
...
legend(1.032*max(3.8),(1+.75)/2,xjust=1,yjust=0.2,pch=c(8, 2),
       c("2BG","3BG"),cex=.9)
